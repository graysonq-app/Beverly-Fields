...
const ACTIVITY_COLORS = {
  "Soccer":         { bg: "#22c55e", light: "#dcfce7", text: "#14532d" },
  "Baseball":       { bg: "#f97316", light: "#ffedd5", text: "#7c2d12" },
  "Football":       { bg: "#8b5cf6", light: "#ede9fe", text: "#3b0764" },
  "Lacrosse":       { bg: "#0ea5e9", light: "#e0f2fe", text: "#0c4a6e" },
  "Free Time":      { bg: "#6b7280", light: "#f3f4f6", text: "#111827" },
  "Tennis":         { bg: "#eab308", light: "#fef9c3", text: "#713f12" },
  "Track & Field":  { bg: "#ec4899", light: "#fce7f3", text: "#831843" },
  "Field Hockey":   { bg: "#14b8a6", light: "#ccfbf1", text: "#134e4a" },
};
...
export default function App() {
  ...
  if (loading) return (
    <div style={{ minHeight: "100vh", background: "#f0f4f0", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "Georgia, serif" }}>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f0f4f0", fontFamily: "'Georgia', serif" }}>
      ...
      <header style={{ background: "#1a3a2a", color: "#e8f5e9", padding: "0 24px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 64, boxShadow: "0 2px 12px rgba(0,0,0,0.18)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <span style={{ fontSize: 26 }}>🌿</span>
          <div>
            <div style={{ fontWeight: 700, fontSize: 18 }}>Beverly Community Fields</div>
            <div style={{ fontSize: 11, opacity: 0.55, letterSpacing: 1, textTransform: "uppercase" }}>Public Schedule</div>
          </div>
        </div>
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          {saving && <span style={{ fontSize: 12, color: "#a7c4b5", fontStyle: "italic" }}>Saving…</span>}
          {adminMode && <span style={{ background: "#22c55e", color: "#fff", borderRadius: 20, padding: "3px 12px", fontSize: 12, fontWeight: 700 }}>ADMIN</span>}
          <button onClick={() => adminMode ? setAdminMode(false) : setShowLogin(true)}
            style={{ background: adminMode ? "#dc2626" : "#2d6a4f", color: "#fff", border: "none", borderRadius: 8, padding: "7px 18px", cursor: "pointer", fontSize: 13, fontFamily: "inherit", fontWeight: 600 }}>
      ...
      {error && (
        <div style={{ background: "#fef2f2", borderBottom: "2px solid #dc2626", padding: "10px 24px", display: "flex", gap: 12, alignItems: "center" }}>
          <span style={{ color: "#dc2626", fontSize: 13 }}>⚠️ {error}</span>
          <button onClick={fetchEvents} style={{ fontSize: 12, color: "#dc2626", background: "none", border: "1px solid #dc2626", borderRadius: 6, padding: "3px 10px", cursor: "pointer" }}>Retry</button>
        </div>
      )}

      <div style={{ background: "#1a3a2a", padding: "0 24px", display: "flex", gap: 3, flexWrap: "wrap", borderBottom: "3px solid #2d6a4f" }}>
        {FIELDS.map((f, i) => (
          <button key={i} onClick={() => setSelectedField(i)} style={{
            background: selectedField === i ? "#f0f4f0" : "transparent",
            color: selectedField === i ? "#1a3a2a" : "#a7c4b5",
            border: "none", borderRadius: "8px 8px 0 0", padding: "9px 18px",
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 24px 6px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span style={{ fontWeight: 700, fontSize: 15, color: "#1a3a2a" }}>
            {weekDates[0].toLocaleDateString("en-US", { month: "long", day: "numeric" })} –{" "}
            {weekDates[6].toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}
          </span>
          <button onClick={() => setWeekOffset(w => w + 1)} style={navBtn}>›</button>
          {weekOffset !== 0 && <button onClick={() => setWeekOffset(0)} style={{ ...navBtn, fontSize: 12, padding: "4px 12px", background: "#e8f5e9", color: "#2d6a4f" }}>Today</button>}
        </div>
        {adminMode && (
          <button onClick={() => { setNewEvent({ ...blankEvent(), field: selectedField }); setShowAddModal(true); }}
            style={{ background: "#22c55e", color: "#fff", border: "none", borderRadius: 8, padding: "8px 20px", cursor: "pointer", fontSize: 14, fontFamily: "inherit", fontWeight: 700 }}>
            + Add Event
          </button>
        )}
      </div>
      ...
      {hasSplitOnField && (
        <div style={{ padding: "0 24px 6px", display: "flex", gap: 12, alignItems: "center" }}>
          ...
          {["N = North Half", "S = South Half"].map(l => (
            <span key={l} style={{ fontSize: 11, background: "#e8f5e9", border: "1px solid #c8e6c9", borderRadius: 4, padding: "1px 7px", color: "#2d6a4f", fontWeight: 700 }}>{l}</span>
          ))}
        </div>
      )}

      <div style={{ padding: "0 24px 40px", overflowX: "auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: "52px repeat(7, 1fr)", background: "#fff", borderRadius: 14, boxShadow: "0 4px 24px rgba(0,0,0,0.07)", overflow: "hidden", border: "1px solid #d4e6d4", minWidth: 700 }}>
          <div style={{ background: "#e8f5e9", borderBottom: "2px solid #c8e6c9" }} />
          {weekDates.map((date, di) => (
            <div key={di} style={{ background: isTodayCol(date) ? "#1a3a2a" : "#e8f5e9", color: isTodayCol(date) ? "#fff" : "#2d6a4f", textAlign: "center", padding: "10px 0 8px", borderBottom: "2px solid #c8e6c9", borderLeft: di > 0 ? "1px solid #c8e6c9" : "none", fontWeight: 700 }}>
              <div style={{ letterSpacing: 1, textTransform: "uppercase", fontSize: 11, opacity: 0.7 }}>{DAY_NAMES[di]}</div>
          ...
          {HOURS.map((hour, hi) => (
            <>
              <div key={`t${hour}`} style={{ background: hi%2===0?"#f7fbf7":"#fff", borderTop: "1px solid #e8f0e8", padding: "4px 6px 0", fontSize: 11, color: "#7aab8a", textAlign: "right", fontFamily: "monospace", userSelect: "none" }}>{fmt(hour)}</div>
              {weekDates.map((wdate, di) => {
                ...
                return (
                  <div key={`${di}-${hour}`} style={{ background: isTodayCol(wdate)?(hi%2===0?"#f0fff4":"#f7fef7"):(hi%2===0?"#f7fbf7":"#fff"), borderTop: "1px solid #e8f0e8", borderLeft: "1px solid #e8f0e8", minHeight: 44, position: "relative", padding: 2 }}>
                    ...
                    {hasSplit && <>
                      <div style={{ position: "absolute", left: "50%", top: 3, bottom: 3, width: 1, background: "#c8e6c9", zIndex: 1 }} />
      ...
      {showLogin && (
        <Modal onClose={() => { setShowLogin(false); setLoginError(""); setAdminPass(""); }}>
          <h2 style={{ margin: "0 0 8px", color: "#1a3a2a", fontSize: 20 }}>Admin Login</h2>
          ...
          {loginError && <div style={{ color: "#dc2626", fontSize: 12, marginTop: 4 }}>{loginError}</div>}
      ...
      {showAddModal && (
        <Modal onClose={() => setShowAddModal(false)} wide>
          <h2 style={{ margin: "0 0 4px", color: "#1a3a2a", fontSize: 20 }}>Add Recurring Event</h2>
      ...
      {editEvent && (
        <Modal onClose={() => setEditEvent(null)} wide>
          <h2 style={{ margin: "0 0 16px", color: "#1a3a2a", fontSize: 20 }}>Edit Event</h2>
      ...
      {cancelTarget && (
        <Modal onClose={() => setCancelTarget(null)}>
          <h2 style={{ margin: "0 0 8px", color: "#1a3a2a", fontSize: 18 }}>Cancel This Occurrence?</h2>
          <p style={{ margin: "0 0 6px", color: "#374151", fontSize: 13 }}><strong>{cancelTarget.ev.label}</strong></p>
          ...
          <div style={{ background: "#fef9c3", border: "1px solid #eab308", borderRadius: 8, padding: "10px 14px", fontSize: 12, color: "#713f12", marginBottom: 16 }}>
...
function EventBlock({ ev, adminMode, onEdit, onDelete, onCancelOccurrence, left, right, badge }) {
  ...
  return (
    <div style={{ position: "absolute", left, right, top: 3, height, zIndex: showMenu ? 10 : 2 }}>
      <div onClick={() => adminMode && setShowMenu(m => !m)} style={{ position: "absolute", inset: 0, background: colors.light, border: `2px solid ${colors.bg}`, borderRadius: 7, padding: "3px 6px", cursor: adminMode ? "pointer" : "default", overflow: "hidden", boxShadow: `0 2px 6px ${colors.bg}33` }}>
        {badge && <span style={{ position: "absolute", top: 3, left: 4, background: colors.bg, color: "#fff", fontSize: 9, fontWeight: 800, borderRadius: 3, padding: "1px 4px" }}>{badge}</span>}
      ...
      {adminMode && showMenu && (
        <div onClick={e => e.stopPropagation()} style={{ position: "absolute", top: "100%", left: 0, zIndex: 20, background: "#fff", border: "1.5px solid #d1d5db", borderRadius: 10, boxShadow: "0 8px 24px rgba(0,0,0,0.13)", minWidth: 190, overflow: "hidden", marginTop: 3 }}>
          <div style={{ padding: "6px 12px", fontSize: 11, color: "#9ca3af", borderBottom: "1px solid #f3f4f6", fontWeight: 700, letterSpacing: 0.5 }}>ADMIN ACTIONS</div>
          <MenuBtn icon="✏️" label="Edit recurring event"  onClick={() => { onEdit();              setShowMenu(false); }} />
          <MenuBtn icon="🚫" label="Cancel this date only" onClick={() => { onCancelOccurrence(); setShowMenu(false); }} color="#b45309" />
          <MenuBtn icon="🗑️" label="Delete entire series"  onClick={() => { onDelete();           setShowMenu(false); }} color="#dc2626" />
...
function MenuBtn({ icon, label, onClick, color = "#374151" }) {
  const [hover, setHover] = useState(false);
  return (
    <button onClick={onClick} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}
      style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", background: hover ? "#f9fafb" : "transparent", border: "none", padding: "9px 14px", cursor: "pointer", fontSize: 13, color, fontFamily: "inherit", textAlign: "left", borderBottom: "1px solid #f3f4f6" }}>
      ...
      <div>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {DAY_NAMES.map((d, i) => {
            ...
            return <button key={i} onClick={() => toggleDay(i)} style={{ background: active?"#1a3a2a":"#f3f4f6", color: active?"#fff":"#6b7280", border: `1.5px solid ${active?"#1a3a2a":"#d1d5db"}`, borderRadius: 8, padding: "5px 10px", cursor: "pointer", fontSize: 12, fontFamily: "inherit", fontWeight: active?700:400 }}>{d}</button>;
          })}
        </div>
        {event.recur_days.length === 0 && <div style={{ color: "#dc2626", fontSize: 11, marginTop: 4 }}>Select at least one day</div>}
      </div>
...
function Modal({ children, onClose, wide }) {
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }} onClick={onClose}>
      <div style={{ background: "#fff", borderRadius: 16, padding: 28, maxWidth: wide?480:400, width: "92%", boxShadow: "0 20px 60px rgba(0,0,0,0.2)", maxHeight: "90vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
        {children}
      </div>
    </div>
  );
...
const navBtn       = { background: "#e8f5e9", color: "#2d6a4f", border: "1.5px solid #c8e6c9", borderRadius: 8, padding: "5px 14px", cursor: "pointer", fontSize: 18, fontFamily: "inherit", fontWeight: 700, lineHeight: 1 };
const primaryBtn   = { background: "#1a3a2a", color: "#fff", border: "none", borderRadius: 8, padding: "9px 22px", cursor: "pointer", fontSize: 14, fontFamily: "inherit", fontWeight: 700, flex: 1 };
const secondaryBtn = { background: "#f3f4f6", color: "#374151", border: "none", borderRadius: 8, padding: "9px 22px", cursor: "pointer", fontSize: 14, fontFamily: "inherit", fontWeight: 600 };
const inputStyle   = { width: "100%", padding: "8px 10px", borderRadius: 8, border: "1.5px solid #d1d5db", fontSize: 13, fontFamily: "inherit", boxSizing: "border-box", marginTop: 4, background: "#f9fafb" };
